...
const mapPage = async function(page, link, frist, option) {
  ...
  const html = await page.evaluate(config => {
    ...
    const timeClass =
      'div.Mid2_L > div.Mid2L_ctt.block > div.Mid2L_tit .detail';
    const localPage =
      (document.querySelector('.page_css b') as any) &&
      (document.querySelector('.page_css b') as any).innerText;
    const time =
      (document.querySelector(timeClass) as any) &&
      (document.querySelector(timeClass) as any).innerText;
    document.querySelectorAll('.page_css a') &&
      Array.from(document.querySelectorAll('.page_css a')).find(
        (item: any, index, arr) => {
    ...
    Array.from(document.querySelectorAll('a')).forEach(item => {
      item.href = '';
      if (
        item.innerText.includes('Steam') ||
        item.innerText.includes('地址') ||
        item.innerText.includes('游民商城') ||
        item.innerText.includes('原文查看')
      ) {
        item.remove();
      }
    });
    ...
    const content = document
      ...
      .innerHTML.replace(/游民星空/g, 'Acfun');
    return {
      time,
      localPage,
      totalPage,
  ...
  if (!html.hasnextPage) {
    html.content +=
      '<p>来源地址：'+link.href+'</p><p><img style="max-width: 700px" class="emotion-icon ubb-emotion" src="https://ali2.a.yximgs.com/bs2/emotion/1587040894630third_party_b35465986.png">大哥大嫂过年好，欢迎大家投香蕉<img style="max-width: 700px" class="emotion-icon ubb-emotion" src="https://ali2.a.yximgs.com/bs2/emotion/1587040895082third_party_b35465992.png"></p>';
  }
...
export const getContent = async function(option) {
  ...
  const link = await page.evaluate(config => {
    ...
    if (config.url === config.urlNews) {
      title = document
        .querySelector(
          'body > div.Mid > div.Mid2 > div.Mid2_L > div.Mid2L_con.block > ul > li:nth-child(1) > div.tit > a.tt',
        )
        .innerHTML.replace(/游民星空/g, 'Acfun');
      href = document
        .querySelector(
      ...
      tag = document.querySelector(
        'body > div.Mid > div.Mid2 > div.Mid2_L > div.Mid2L_con.block > ul > li:nth-child(1) > div.tit > a.dh',
      ).innerHTML;
      id = href.match(/\d+/g).join();
    } else if (config.url === config.urlEnt) {
      title = document
        .querySelector('div.Mid2_L ul > li:nth-child(2) > div.tit > a.tt')
        .innerHTML.replace(/游民星空/g, 'Acfun');
      href = document
        .querySelector('div.Mid2_L ul > li:nth-child(2) > div.tit > a.tt')
        .getAttribute('href');
      tag = document.querySelector(
        'div.Mid2_L ul > li:nth-child(2) > div.tit > a.dh',
      ).innerHTML;
      id = href.match(/\d+/g).join();
    }
    return {
